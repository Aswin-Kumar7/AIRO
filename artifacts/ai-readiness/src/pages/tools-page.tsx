...
function CopyButton({ text, label = "Copy" }: { text: string; label?: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-zinc-300 hover:text-slate-800 dark:text-slate-200 transition-colors"
    >
      {copied ? (
...
function LlmsTab({ storeId }: { storeId: string }) {
  ...
  if (isLoading || isFetching) {
    return (
      <div className="flex items-center justify-center py-20 gap-3">
        ...
        <span className="text-sm text-slate-400 dark:text-zinc-400">Generating llms.txt…</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Info strip */}
      <div className="flex gap-3 bg-slate-50 dark:bg-white/5 border border-slate-200/60 dark:border-white/10 rounded-xl px-4 py-3 shadow-sm">
        <Info className="w-4 h-4 text-slate-400 dark:text-zinc-400 flex-shrink-0 mt-0.5" />
        <p className="text-xs text-slate-500 dark:text-zinc-300 leading-relaxed">
          <span className="font-semibold text-slate-700 dark:text-slate-200 uppercase tracking-tighter text-[10px] mr-1.5">llms.txt</span> is the emerging standard
      ...
      {data?.content ? (
          ...
          <div className="bg-white dark:bg-[#080808] rounded-[16px] border border-slate-200/60 dark:border-white/10 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">llms.txt</span>
              </div>
            ...
            <pre className="px-4 py-4 text-[11px] font-mono text-slate-600 dark:text-zinc-200 whitespace-pre-wrap leading-relaxed overflow-x-auto">
              {data.content}
            </pre>
          </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-white/10 flex items-center justify-center mb-4">
            <FileText className="w-6 h-6 text-slate-400 dark:text-zinc-400" />
          </div>
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">Generate your llms.txt</p>
...
function FaqTab({ storeId }: { storeId: string }) {
  ...
  if (!hasRun && !perception) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-white/10 flex items-center justify-center mb-4">
          <FileQuestion className="w-6 h-6 text-slate-400 dark:text-zinc-400" />
        </div>
        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">FAQ health analysis</p>
        <p className="text-xs text-slate-400 dark:text-zinc-400 max-w-xs mb-5 leading-relaxed">
  ...
  if (percLoading || isFetching) {
    return (
      <div className="flex items-center justify-center py-20 gap-3">
        <Loader2 className="w-4 h-4 animate-spin text-slate-300" />
        <span className="text-sm text-slate-400 dark:text-zinc-400">Analyzing FAQ…</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {perception && (
        <div className="bg-white dark:bg-[#080808] rounded-[16px] border border-slate-200/60 dark:border-white/10 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4">
            ...
            {faq?.found && (
              <span className="text-[10px] font-semibold text-slate-500 dark:text-zinc-300 bg-slate-100 dark:bg-white/10 px-2 py-1 rounded-full">
          ...
          {perception.unansweredQuestions.length > 0 && (
            <>
              <div className="border-t border-slate-100 dark:border-white/5 px-5 py-2.5 bg-slate-50/60">
                <p className="text-[10px] font-semibold text-slate-400 dark:text-zinc-400 uppercase tracking-wide">
              ...
              <div className="divide-y divide-slate-100 dark:divide-white/5">
                {perception.unansweredQuestions.map((q, i) => (
                  <div key={i} className="flex items-start gap-3 px-5 py-2.5">
                    ...
                    <p className="text-xs text-slate-600 dark:text-zinc-200">{q}</p>
                  </div>
                ))}
              </div>
            </>
      ...
      {faq?.unansweredTopics && faq.unansweredTopics.length > 0 && (
        <div className="bg-white dark:bg-[#080808] rounded-[16px] border border-amber-200/60 shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-3 border-b border-amber-100 bg-amber-50/40">
            ...
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Topics not covered</span>
          </div>
          <div className="divide-y divide-amber-50">
            {faq.unansweredTopics.map((t, i) => (
              <div key={i} className="flex items-start gap-3 px-5 py-2.5">
                <span className="text-amber-400 text-xs flex-shrink-0 mt-0.5">·</span>
                <p className="text-xs text-slate-600 dark:text-zinc-200">{t}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#080808] rounded-[16px] border border-slate-200/60 dark:border-white/10 shadow-sm overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            <FileQuestion className="w-4 h-4 text-slate-400 dark:text-zinc-400" />
            <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">FAQPage JSON-LD schema</span>
            {schema && (
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-200">
                Generated
              </span>
            )}
          </div>
          ...
          <div className="border-t border-slate-100 dark:border-white/5">
            {schemaLoading || schemaFetching ? (
              <div className="flex items-center justify-center py-12 gap-3">
                <Loader2 className="w-4 h-4 animate-spin text-slate-300" />
                <span className="text-sm text-slate-400 dark:text-zinc-400">Generating schema…</span>
              </div>
            ) : schema ? (
              <div className="p-5 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {schema.questions.map((q: { question: string; answer: string }, i: number) => (
                    <div key={i} className="border border-slate-100 dark:border-white/5 rounded-lg px-4 py-3">
                      <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 mb-1">{q.question}</p>
                      <p className="text-xs text-slate-500 dark:text-zinc-300 leading-relaxed">{q.answer}</p>
                    </div>
                  ))}
                </div>

                {/* Code snippet */}
                <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg overflow-hidden">
                  ...
                  <pre className="px-3 py-3 text-[10px] font-mono text-slate-600 dark:text-zinc-200 overflow-x-auto whitespace-pre-wrap">
                    {snippet}
                  </pre>
                </div>
                ...
                <button
                  ...
                  className="text-xs text-slate-400 dark:text-zinc-400 hover:text-slate-600 dark:text-zinc-200 transition-colors"
                >
                  Regenerate
                </button>
              </div>
  ...
  return (
    <AppLayout>
      <div className="p-6 max-w-3xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Tools</h1>
        ...
        <Tabs defaultValue="llms">
          <TabsList className="mb-8 bg-slate-100/80 dark:bg-white/5 p-1 h-auto rounded-[12px] border dark:border-white/5 shadow-inner">
            ...
            <TabsTrigger
              ...
              className="text-[13px] font-bold h-9 px-6 gap-2 data-[state=active]:bg-white dark:bg-transparent data-[state=active]:dark:bg-white/10 data-[state=active]:shadow-sm data-[state=active]:text-emerald-700 dark:data-[state=active]:text-white rounded-[9px] transition-all"
            >
              <FileQuestion className="w-4 h-4" /> FAQ Schema
            </TabsTrigger>
          </TabsList>
